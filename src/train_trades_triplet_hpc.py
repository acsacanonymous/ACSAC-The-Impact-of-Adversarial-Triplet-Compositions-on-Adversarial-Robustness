...
def main(args):
    ...
    for epoch in range(args.epochs):
        ...
        for i, (X, y) in enumerate(train_loader):
            ...
            _, features_benign = model(X)

            embeddings_benign = F.normalize(features_benign)

            loss_trades, batch_metrics, X_adv = trades_loss(model, X, y, opt, step_size=alpha, epsilon=epsilon,
            ...
            loss = trades_weight * loss_trades + triplet_weight * loss_triplet
            ...
            loss_trades_tot += loss_trades * y.size(0)
            ...
            if (i + 1) % 10 == 0:
                ...
                print(
                    ...
                    f"{loss_trades_tot / train_n} \t {no_triplets} \t {loss_triplet_tot / no_triplets} \t"
                    f"{loss_triplet_tot / train_n}"
                )
                logging.info(
                    f"{epoch} \t {lr} \t {train_n} \t {train_loss / train_n} \t "
                    f"{correct_clean / train_n} \t {correct_adv / train_n} \t"
            ...
            with ema.average_parameters():
                with torch.no_grad():
                    for i, (X, y) in enumerate(test_loader):
                        ...
                        _, _, X_adv = trades_loss(model, X, y, opt, step_size=alpha, epsilon=epsilon,
                                                  perturb_steps=args.attack_iters, beta=args.beta,
                                                  classifier_ae=args.class_ae)
...
def get_args():
    ...
    parser.add_argument('--mining', default="all", type=str, choices=["all", "batch_hard"], help='all/batch-hard')
    parser.add_argument('--triplet_w', default=0.5, type=float, help='Weight for triplet loss')
    parser.add_argument('--trades_w', default=1, type=float, help='Weight for trades loss')
